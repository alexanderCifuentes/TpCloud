...
const fs = require('fs');
const ManejoErrores = require('./Error');
...
app.post('/api/albums',Validations.validate(Validations.validationAlbum), routehelper(async (req, res) =>{
...
app.get('/api/tracks', routehelper(async (req,res) =>{
  
  const unqfy = getUNQfy();
  res.send(unqfy.getTracks());
}))
